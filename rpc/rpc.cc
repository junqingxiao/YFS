		...
		TO to)
{
	...
	caller ca(0, &rep);
	{
		...
		//xid_rep_window_.front() is always latest received reply xid
	...
	for (clt = reply_window_.begin(); clt != reply_window_.end(); clt++)
	{
		if (clt->first == clt_nonce)
		{
			if(clt->second.size())
			{
				...
				bool forgottenOrNot=true;
				for(it=clt->second.begin();it != clt->second.end();it++)
				{
					if((*it).xid == xid)
					{//if the xid already exist
						if((*it).cb_present)
						{
							return INPROGRESS;
						}
						...
						{
							//use *b,*sz to write the reply that recorded
							*b=(*it).buf;
							*sz=(*it).sz;
							return DONE;
						}
				...
				if(it == clt->second.end())
				{
					...
					{//need to add_reply to map and set cb_present=true;
						reply_t *replyT=new reply_t(xid);
						replyT->buf=NULL;
						replyT->sz=0;
						replyT->cb_present=true;
						clt->second.push_back(*replyT);
						delete replyT;
						return NEW;
					}
				}
